...
$repo = 'D:\1C\DevTool'
$updateCF = 'C:\Users\admin\Downloads\Инструменты разработчика Конфигурация 6.44.2.cf'
...
$vendor |
    %{
        ...
        IF (Test-Path (Join-Path $folder "vendorNEW\$f")) {
            ...
            Start-Process -FilePath "C:\Program Files\KDiff3\kdiff3.exe" -Wait -ArgumentList """$(Join-Path $folder "vendor\$f")"" ""$(Join-Path $folder "main\$f")"" ""$(Join-Path $folder "vendorNEW\$f")"" -o ""$(Join-Path $folder "mainNEW\$f")"""
